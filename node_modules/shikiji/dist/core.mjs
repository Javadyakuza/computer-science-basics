...
function codeToHast(internal, code, options, transformerContext = {
  ...
  if ("themes" in options) {
    ...
    tokens = themeTokens.map((line) => line.map((token) => flattenToken(token, themesOrder, cssVariablePrefix, defaultColor)));
...
function codeToHtml(internal, code, options) {
  ...
  let result = toHtml(codeToHast(internal, intput, options, context));
  for (const transformer of options.transformers || [])
...
export { addClassToHast, codeToHast, codeToHtml, codeToThemedTokens, codeToTokensWithThemes, createSingletonShorthands, createdBundledHighlighter, getHighlighterCore, getShikiContext, getShikiInternal, isPlaintext, isSpecialLang, loadWasm, toArray, toShikiTheme, tokenizeAnsiWithTheme };
